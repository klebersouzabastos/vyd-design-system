...
> ⚠️ **Componentes React (`@vyd/react`) NÃO estão no npm** (por ora vivem só no
> repositório). Em React você usa as **classes `.vyd-*`** e os **tokens** — que é o
> mesmo que os componentes fazem por baixo. Se quiser os componentes React
> empacotados, veja "React components" no fim.
...
## React components (`@vyd/react`) — status

Os wrappers React (`Button`, `Field`, `Dialog`, `Table`, gráficos…) existem no
repositório (`react/`), mas **ainda não são publicados no npm**. Enquanto isso, em
React você usa as classes `.vyd-*` diretamente (é o que os componentes fazem).

Quer os componentes React instaláveis (`npm install …`)? Dá pra publicar como um
segundo pacote (ex.: `vyd-react`, já que o escopo `@vyd` está indisponível). É um
passo à parte — peça e a gente encaminha.
